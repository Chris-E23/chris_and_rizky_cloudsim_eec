...
static map<MachineId_t, long> machines_and_instructions;
...
void GreedyScheduler::NewTask(Time_t now, TaskId_t task_id)
{
    ...
    for (MachineId_t machine_id = 0; machine_id < total_machines; machine_id++)
    {
        ...
        if (new_task_info.required_sla == SLA0 && MachineMIPS(machine_id) < max_mips * 0.8f)
            continue;
        if (new_task_info.required_sla == SLA1 && MachineMIPS(machine_id) < max_mips * 0.5f)
            continue;
    ...
    if (best_machine == INVALID_MACHINE)
    {
        best_util = std::numeric_limits<float>::max();
        for (MachineId_t machine_id = 0; machine_id < total_machines; machine_id++)
        {
            // Assign tasks based on the info at hand
            // Choose machines with least number of tasks and most memory available
    ...
    machines_and_instructions[best_machine] += new_task_info.remaining_instructions;
...
void GreedyScheduler::TaskComplete(Time_t now, TaskId_t task_id)
{
    ...
    machines_and_instructions[vm_info.machine_id] -= task_info.total_instructions;
}
